...
interface AddBlockFormProps {
  ...
  /** Sponsor catalog items (from connected sponsors' libraries) to add as blocks. */
  sponsorCatalogItems?: {
    id: string;
    sponsor_id: string;
    type: string;
...
export function AddBlockForm({
  ...
  const [showSponsorCatalog, setShowSponsorCatalog] = useState(false);
  const [addingFromLibrary, setAddingFromLibrary] = useState<string | null>(null);
  const [addingFromSponsorCatalog, setAddingFromSponsorCatalog] = useState<string | null>(null);
  ...
  const reset = () => {
    ...
    setShowSponsorCatalog(false);
    setAddingFromLibrary(null);
    setAddingFromSponsorCatalog(null);
  };

  const handleAddFromSponsorCatalog = async (itemId: string) => {
    setAddingFromSponsorCatalog(itemId);
    const result = await addSponsorLibraryBlockToFanflet(fanfletId, itemId);
    setAddingFromSponsorCatalog(null);
    if (result.error) {
      toast.error(result.error);
      return;
    }
    toast.success("Sponsor resource added to fanflet");
    reset();
    router.refresh();
    onAdded();
  };
  ...
  if (!open) {
    return (
      <div className="flex gap-2">
        ...
        {sponsorCatalogItems.length > 0 && (
          <Button
            variant="outline"
            className="border-dashed border-2 border-slate-200 hover:border-teal-500 hover:bg-teal-500/5 gap-2 flex-1"
            onClick={() => { setOpen(true); setShowSponsorCatalog(true); }}
          >
        ...
        <div className="flex items-center gap-2 mt-4">
          ...
          {sponsorCatalogItems.length > 0 && (
            <Button
              variant="ghost"
              size="sm"
              onClick={() => setShowSponsorCatalog(true)}
              className="text-teal-600"
            >
              <Library className="w-4 h-4 mr-1" />
              or from Sponsor catalog
            </Button>
          )}
        </div>
      </div>
