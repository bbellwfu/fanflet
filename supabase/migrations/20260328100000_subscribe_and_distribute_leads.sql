-- Atomically creates a subscriber and distributes leads to all associated sponsors
...
BEGIN
  ...
  IF p_sponsor_consent THEN
    FOR v_sponsor_id IN (
      -- Union unique sponsors from explicit and implicit sources
      SELECT fs.sponsor_id 
      FROM public.fanflet_sponsors fs 
      WHERE fs.fanflet_id = p_fanflet_id
      UNION
      SELECT rb.sponsor_account_id 
      FROM public.resource_blocks rb 
      WHERE rb.fanflet_id = p_fanflet_id AND rb.sponsor_account_id IS NOT NULL
    ) LOOP
      ...
      IF EXISTS (
        ...
        ON CONFLICT DO NOTHING; -- Avoid duplicates if they rerun
