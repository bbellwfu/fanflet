    ...
    <div className="h-full flex flex-col bg-sidebar border-r border-border-subtle">
      ...
      <nav className="flex-1 overflow-y-auto px-3 pb-2">
        {navSections.map((section) => {
          ...
          return (
            <div key={section.label || "_top"} className={section.label ? "mt-5" : ""}>
              {section.label && (
                <p className="px-3 pb-1.5 text-[10px] font-semibold uppercase tracking-wider text-fg-muted">
                  {section.label}
                </p>
              )}
              <div className="space-y-0.5">
                {visibleItems.map((item) => (
                  <NavItemLink key={item.id} item={item} pathname={pathname} />
                ))}
              </div>
            </div>
          );
