...
export async function listSponsorCampaigns(): Promise<{
  ...
  const { data: campaigns, error } = await supabase
    ...
    .select("id, sponsor_id, name, description, start_date, end_date, status, crm_reference, created_at, updated_at")
  ...
  if (ids.length > 0) {
    const [speakersRes, libRes] = await Promise.all([
      ...
      supabase.from("sponsor_resource_library").select("campaign_id").eq("sponsor_id", sponsorId).in("campaign_id", ids),
    ]);
...
export async function createSponsorCampaign(params: {
  ...
  const speakerIds = params.speaker_ids ?? [];
...
export async function updateSponsorCampaign(
  ...
  if (params.speaker_ids !== undefined) {
    await supabase.from("sponsor_campaign_speakers").delete().eq("campaign_id", campaignId);
    const speakerIds = params.speaker_ids.filter(Boolean);
    if (speakerIds.length > 0) {
